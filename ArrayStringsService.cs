...
public static class ArrayStringsService
{
    ...
    /// Works but does not go through performance validation
    /// </summary>
    ...
    public static int[] ProductExceptSelfV2(int[] nums)
    {
        ...
        else if (zeroCount = 1)
        {
            return nums.Select(x =>
            {
                if (x == 0)
                {
                    return nums.Where(x => x != 0).Aggregate((x, y) => x * y, 1);
                }
                else
                {
                    return 0;
    ...
    private static int[] Multiply(int[] nums, int product)
    {
